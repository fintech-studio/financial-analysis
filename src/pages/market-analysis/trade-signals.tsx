...
const AnimatedSelectWrapper: React.FC<AnimatedSelectWrapperProps> = ({
  ...
  <div className={`relative ${className}`}>
    ...
    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
      <svg
        className="w-4 h-4 text-gray-500 transition-all duration-300 group-hover:text-gray-700 group-focus-within:text-gray-700 group-focus-within:rotate-180"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
...
const TradeSignalsPage: React.FC = () => {
  ...
  const Title = "進階交易訊號分析";
  const Subtitle = "提供股票等交易訊號查詢與分析工具";
  const Description =
    "查詢並分析技術指標產生的交易訊號，協助投資人做出更明智的交易決策";
  const panelTitle = Object.values(results)
    .reduce((acc, arr) => acc + arr.length, 0)
    .toString();
  const panelSubtitle = "資料筆數";
  const panelTitle2 = "多市場";
  const panelSubtitle2 = "支援類型";
  const Form = (
    <form
    ...
    >
      <div className="flex-1 min-w-0 relative">
        <input
          type="text"
          className="w-full px-6 py-4 bg-white/10 backdrop-blur-sm border-2 border-white/20 rounded-2xl text-white placeholder-blue-200 focus:outline-none focus:border-white/40 transition-all duration-300 text-base min-w-80"
          placeholder="輸入市場代號..."
          value={symbol}
      ...
      <div className="flex flex-col sm:flex-row gap-4 shrink-0">
        ...
        >
          ...
          <option value="both" className="bg-white text-gray-900 py-2">
            📈 日線&小時線
          </option>
        </AnimatedSelect>
        <AnimatedSelect
        ...
        >
          ...
          <option value="market_crypto" className="bg-white text-gray-900 py-2">
            🌐 加密貨幣
          </option>
          <option value="market_forex" className="bg-white text-gray-900 py-2">
            💱 外匯
          </option>
          <option value="market_etf" className="bg-white text-gray-900 py-2">
            📊 ETF
          </option>
          <option
          ...
          <option value="market_index" className="bg-white text-gray-900 py-2">
            📉 指數
          </option>
        </AnimatedSelect>
        <button
                      ...
                      <div>
                        <div className="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-xl">
                          <input
                            ...
                            className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 flex-1 max-w-79"
