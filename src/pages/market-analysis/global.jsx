...
const GlobalMarket = () => {
  ...
  const marketData = {
    ...
    centralBanks: [
      { name: '美聯儲', rate: '5.25-5.50%', stance: '偏鷹派', nextMeeting: '2024/03/20' },
      { name: '歐洲央行', rate: '4.50%', stance: '中性', nextMeeting: '2024/03/07' },
      { name: '日本央行', rate: '-0.10%', stance: '偏鴿派', nextMeeting: '2024/03/19' },
    ],
    economicIndicators: {
  ...
  const ratesChartData = {
    ...
    datasets: [
      {
        label: '政策利率',
        data: marketData.centralBanks.map(bank => parseFloat(bank.rate.split('-')[0])),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="border-b border-gray-200 mb-8">
          <nav className="-mb-px flex space-x-8">
            {['overview', 'regions', 'commodities', 'economy'].map((tab) => (
              <button
                key={tab}
                className={`${
                  activeTab === tab
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {marketData.overview.indices.map((index) => (
                <div key={index.name} className="bg-white rounded-xl shadow-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">{index.name}</h3>
                    <span className={`text-sm font-medium ${getStatusColor(index.trend)}`}>
                      {index.change}
                    </span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-2xl font-bold text-gray-900">{index.value}</span>
                    <span className="text-sm text-gray-500">{index.region}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {marketData.regions.map((region) => (
                <div key={region.name} className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  <div className="flex items-baseline justify-between">
                    <span className="text-2xl font-bold text-gray-900">{region.performance}</span>
                  </div>
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <p className="text-sm text-gray-500 mb-2">{region.description}</p>
                    <div className="text-sm">
                      <span className="text-gray-500">主要市場：</span>
                      <span className="text-gray-900">{region.leadingMarkets.join('、')}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {marketData.overview.commodities.map((commodity) => (
                <div key={commodity.name} className="bg-white rounded-xl shadow-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">{commodity.name}</h3>
                    <span className={`text-sm font-medium ${getStatusColor(commodity.trend)}`}>
                      {commodity.change}
                    </span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-2xl font-bold text-gray-900">{commodity.value}</span>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="h-[300px]">
                <Line options={commodityChartOptions} data={commodityChartData} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow mb-8">
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="text-md font-medium text-gray-900 mb-4">區域經濟概況</h3>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">地區</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">GDP成長</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">通膨率</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {marketData.regions.map((region) => (
                        <tr key={region.name}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{region.gdpGrowth}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{region.inflation}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="bg-white rounded-lg shadow mb-8">
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="text-md font-medium text-gray-900 mb-4">主要央行動態</h3>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">央行</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">政策立場</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">下次會議</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {marketData.centralBanks.map((bank) => (
                        <tr key={bank.name}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{bank.stance}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{bank.nextMeeting}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
