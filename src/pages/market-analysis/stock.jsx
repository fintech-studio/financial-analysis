...
const StockMarket = () => {
  ...
  const marketData = {
    ...
    strategies: {
      ...
      growth: [
        { symbol: '2454', name: '聯發科', growth: 25.3, momentum: 'strong', score: 88 },
        { symbol: '3008', name: '大立光', growth: 18.7, momentum: 'strong', score: 84 },
      ],
      momentum: [
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="flex items-baseline justify-between">
                <span className="text-3xl font-bold text-gray-900">{marketData.overview.index}</span>
                <span className="text-green-500 font-semibold">{marketData.overview.changePercent}</span>
              </div>
              <div className="mt-2 text-sm text-gray-500">
                成交量：{marketData.overview.volume}
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="text-sm text-gray-500">{marketData.overview.upDown}</div>
              ...
              {marketData.sectors.map((sector) => (
                <div key={sector.name} className="bg-white rounded-xl shadow-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`text-sm font-medium ${getStatusColor(sector.strength)}`}>
                      {sector.strength}
                    </span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-2xl font-bold text-gray-900">{sector.performance}</span>
                  </div>
                  <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col justify-center">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">產業權重</h3>
                  <div className="space-y-4">
                    {marketData.sectors.map((sector) => (
                      <div key={sector.name} className="flex items-center justify-between">
                        <span className="text-sm text-gray-500">{sector.name}</span>
                        <span className="text-sm font-medium text-gray-900">{sector.weight}%</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-white rounded-lg shadow mb-8">
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="h-[300px]">
                    <Pie data={sectorRotationData} options={sectorChartOptions} />
                  </div>
                  <div>
                    <h3 className="text-md font-medium text-gray-900 mb-4">產業表現</h3>
                    <div className="space-y-4">
                      {marketData.sectors.map((sector) => (
                        <div key={sector.name} className="flex items-center justify-between">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-gray-900">{marketData.technical.ma5}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">MA10</span>
                    <span className="text-sm font-medium text-gray-900">{marketData.technical.ma10}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">MA20</span>
                    <span className="text-sm font-medium text-gray-900">{marketData.technical.ma20}</span>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-gray-900">{marketData.technical.rsi}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">MACD</span>
                    <span className={`text-sm font-medium ${getStatusColor(marketData.technical.macd)}`}>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-gray-900">
                      {marketData.technical.support} / {marketData.technical.resistance}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {getFilteredStocks().map((stock) => (
                    <tr key={stock.symbol}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{stock.score}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {selectedStrategy === 'value' && `P/E: ${stock.pe}, P/B: ${stock.pb}`}
                        {selectedStrategy === 'growth' && `成長率: ${stock.growth}%, 動能: ${stock.momentum}`}
                        {selectedStrategy === 'momentum' && `RSI: ${stock.rsi}, 趨勢: ${stock.trend}`}
                      </td>
                    </tr>
                  ))}
                </tbody>
