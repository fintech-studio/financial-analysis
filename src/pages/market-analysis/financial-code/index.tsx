...
const FinancialCodeTestPage = () => {
  ...
  const Icon = DocumentMagnifyingGlassIcon;
  const Title = "金融代號查詢";
  const Subtitle = "查詢股票、加密貨幣、ETF、期貨等金融商品代號";
  const Description =
    "快速查詢各類金融商品的代號資訊，包含股票、指數、ETF 等資料";
  const panelTitle = results.length.toString();
  const panelSubtitle = "查詢結果";
  const panelTitle2 = "2025-06-26";
  const panelSubtitle2 = "資料更新";
  const Form = (
    <>
      <form
        onSubmit={(e) => {
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-gray-200/50 overflow-hidden">
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-center">
                        ...
                        <thead>
                          <tr className="bg-linear-to-r from-blue-100 to-indigo-100">
                            {columns.map((col) => (
                              <th
                                key={col.key}
                        ...
                        <tbody>
                          {paginatedResults.map((row, idx) => (
                            <tr
                              ...
                              className={`hover:bg-blue-50/80 transition-all duration-300 hover:shadow-lg ${
                  ...
                  <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 p-6">
                    <div className="flex flex-wrap justify-center items-center gap-3">
                      ...
                      <div className="flex items-center gap-2 bg-linear-to-r from-blue-50 to-indigo-50 rounded-xl px-4 py-2 border border-blue-200 shadow-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <button
                          className="px-4 py-2 rounded-xl bg-linear-to-r from-blue-600 to-indigo-600 text-white font-medium disabled:opacity-50 hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-sm disabled:cursor-not-allowed"
                          onClick={handleJump}
                          disabled={
                            !jumpPage ||
                            Number(jumpPage) < 1 ||
                    ...
                    <div className="mt-4 text-center">
                      <span className="inline-flex items-center px-6 py-3 bg-linear-to-r from-indigo-50 to-blue-50 text-indigo-700 rounded-2xl border border-indigo-200 font-medium shadow-sm">
                        <svg
