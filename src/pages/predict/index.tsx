...
import { ChatBubbleLeftIcon, Cog6ToothIcon, MagnifyingGlassIcon } from "@heroicons/react/24/outline";
import Footer from "../../components/Layout/Footer";
import PredictionLineChart from "../../components/Charts/dickchart";
...
const DiscussionPage: React.FC = () => {
  ...
  const [dbConfig, setDbConfig] = useState<DatabaseConfig>({
    ...
    options: {
      encrypt: process.env.NEXT_PUBLIC_DB_ENCRYPT === "true" || true,
      trustServerCertificate: process.env.NEXT_PUBLIC_DB_TRUST_CERT === "true" || true,
    }
  });
  ...
  // 轉換預測數據為折線圖格式
  const convertPredictionToLineData = (predictionData: PredictionData): LineChartData[] => {
  ...
  // 轉換評估數據為折線圖格式
  const convertEvaluationToLineData = (evaluationData: EvaluationData): LineChartData[] => {
    const baseDate = new Date();
    const chartData: LineChartData[] = [];
    ...
    // 同時處理真實值和預測值
    const maxLength = Math.max(evaluationData.true_value.length, evaluationData.predict.length);
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      // 真實值作為主線
      const trueValue = i < evaluationData.true_value.length 
        ? evaluationData.true_value[i] 
        : 0;
      
      // 預測值作為預測線
      const predictValue = i < evaluationData.predict.length 
        ? evaluationData.predict[i] 
        : 0;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          {/* 國家/市場選擇 */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-3">選擇市場</label>
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              股票代碼
              {selectedCountry === "TW" && <span className="text-gray-500 ml-1">(例: 2330)</span>}
              {selectedCountry === "US" && <span className="text-gray-500 ml-1">(例: AAPL)</span>}
              {selectedCountry === "CRYPTO" && <span className="text-gray-500 ml-1">(例: BTC)</span>}
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder={`輸入${countryOptions.find(c => c.value === selectedCountry)?.label}股票代碼`}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">
              熱門{countryOptions.find(c => c.value === selectedCountry)?.label}股票
            </label>
            <div className="flex flex-wrap gap-2">
              {popularStocks[selectedCountry as keyof typeof popularStocks]?.map((symbol) => (
                <button
                  key={symbol}
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="number"
                value={contextLength}
                onChange={(e) => setContextLength(parseInt(e.target.value) || 192)}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setPredictionLength(parseInt(e.target.value) || 12)}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="12"
              />
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <Cog6ToothIcon className="w-6 h-6 text-blue-600" />
              <h2 className="text-xl font-semibold text-gray-900">資料庫連接</h2>
          ...
          {stockSymbol && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">
                已選擇: <span className="font-medium">{countryOptions.find(c => c.value === selectedCountry)?.flag} {stockSymbol}</span>
                {stockData.length > 0 && (
                  <span className="ml-3">
                    • 已載入 {stockData.length} 筆資料，最新價格: {stockData[0]?.toFixed(2)}
                  </span>
                )}
              </p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="mb-8 text-center">
            ...
            <div className="flex justify-center space-x-4">
              ...
              <button
                ...
                disabled={loading || stockData.length < contextLength + predictionLength}
          ...
          {(prediction || evaluation) && (
            <div className="mt-8">
              ...
              {prediction && !evaluation && (
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-4 text-center">AI 預測結果</h3>
                  <PredictionLineChart
              ...
              {evaluation && (
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-4 text-center">模型評估結果</h3>
                  <PredictionLineChart
