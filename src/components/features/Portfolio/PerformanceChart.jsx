...
const PerformanceChart = ({
  ...
  const [compareIndices, setCompareIndices] = useState(["benchmark"]); // 可比較多個指數
  ...
  const calculateVolatility = (dataPoints) => {
    ...
    // 年化波動率 (假設數據是每日數據)
    const annualizedVolatility = Math.sqrt(variance) * Math.sqrt(252); // 252為一年的交易日

    return annualizedVolatility;
  };

  // 簡化的年化回報率計算
  const calculateAnnualizedReturn = (totalReturn, timeRange) => {
    const yearsMapping = {
      "1W": 1 / 52,
      "1M": 1 / 12,
      "3M": 3 / 12,
    ...
    if (highlightPeriods && data.keyPeriods) {
      data.keyPeriods.forEach((period) => {
        if (
          ...
          const highlightData = Array(filteredData.labels.length).fill(null);
  ...
  // 處理基準指標選擇
  const handleBenchmarkToggle = (benchmarkId) => {
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    ...
                    <tr>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium text-green-600">
                        +{(metrics.totalReturn - 16.8).toFixed(1)}%
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium text-green-600">
                        +14.2%
                      </td>
                    ...
                    <tr>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium text-red-600">
                        {(metrics.totalReturn - 21.5).toFixed(1)}%
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium text-green-600">
                        +18.3%
                      </td>
