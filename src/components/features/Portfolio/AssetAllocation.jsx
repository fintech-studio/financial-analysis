...
const AssetAllocation = ({ data }) => {
  ...
  const calculateDeviation = () => {
    ...
    return data.byAssetClass.map((item, index) => {
      const targetItem = targetAllocation.assetClass.find(
        (t) => t.name === item.name
      ) || { name: item.name, percentage: 0 };
      const deviation = item.percentage - targetItem.percentage;
      return {
        name: item.name,
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="h-64 md:h-80">
            {showComparison && activeTab === "assetClass" ? (
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {data.byAssetClass.map((item, index) => {
                        const targetItem = showComparison
                          ...
                          : null;
                        const deviation = showComparison
                    ...
                    >
                      <div className="flex items-center">
                        <div
                          ...
                          style={{
                            backgroundColor: `rgba(${(index * 30) % 255}, ${
                              (index * 50 + 100) % 255
                            }, ${(index * 70 + 50) % 255}, 0.6)`,
                          }}
                        ></div>
