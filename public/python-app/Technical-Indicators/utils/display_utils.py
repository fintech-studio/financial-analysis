...
def print_statistics(stats: Dict[str, Any]):
    ...
    print(f"\n📊 資料庫統計資訊 ({table_name} - {interval})")
    print(f"{'='*50}")
    print(f"總記錄數: {stats.get('total_records', 0):,}")
    print(f"股票數量: {stats.get('unique_symbols', 0)}")

    date_range = stats.get('date_range', {})
    if date_range.get('earliest') and date_range.get('latest'):
        print(f"日期範圍: {date_range['earliest']} ~ {date_range['latest']}")
...
def print_all_statistics(all_stats: Dict[str, Dict[str, Any]]):
    ...
    for interval in interval_order:
        if interval in all_stats:
            ...
            print(f"📈 {table_name} ({interval}):")
            print(f"   記錄數: {total_records:,} 筆")
            print(f"   股票數: {unique_symbols} 個")
    ...
    print(f"📋 總結: {total_tables} 個資料表，共 {total_all_records:,} 筆記錄")
